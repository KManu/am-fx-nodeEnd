...
function login(payload) {
  return UserModel.findOne({ email: payload.email }, 'password')
    .exec()
    .then(next => {
...
/**
 * Creates a user is the email doesnt exist
 * @param {Object} userObject
 * @return {Promise} result
 */
function create(userObject) {
  return UserModel.exists({ email: userObject.email })
    .then(res => {
      if (res === false) {
...
function getByOrgCode(orgCode) {
  return OrgModel.findOne({ code: orgCode })
    .select({ _id: 1 })
    .exec()
    .then(next => {
      if (next !== null) {
        return UserModel.find({ organisation: next._id }).select({
          password: 0
        });
      } else {
        return false;
